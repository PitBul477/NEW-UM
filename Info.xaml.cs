    ...
    public partial class Info : Window
    {
        ...
        private void ComboBox_Info(object sender, SelectionChangedEventArgs e)
        {
            ...
            switch (help.SelectedIndex)
            {
                case 0:
                    // Концепция программы
                    text.Text = $"Данная программа представляет собой упрощенную интернет-версию телевизионной игры «Угадай мелодию». Программа содержит ноты с мелодиями, и при нажатии на них воспроизводится соответствующий музыкальный файл. Воспроизведение музыки осуществляется непосредственно в приложении, и дополнительные плееры или программы, такие как Soundpad, не требуются.\r\nВ первом раунде игры у каждой ноты есть счетчик баллов, который уменьшается. Чтобы заработать максимальное количество баллов, игрокам необходимо как можно раньше нажать на кнопку для остановки мелодии и дать правильный ответ.\r\nВо втором раунде принцип игры меняется: количество нот уменьшается, но счетчик баллов увеличивается от 0 до определенного лимита при проигрывании каждой ноты. В этом раунде баллы начисляются не за отдельные песни, а за всю категорию мелодий.\r\nВ финале игры выходят два игрока, набравших наибольшее количество баллов. В финале подготовлено как минимум одна мелодия, желательно больше. Первый из финалистов, кто угадает мелодию, получает балл. Победителем становится игрок, который после проигрывания всех финальных мелодий наберет наибольшее количество баллов.\r\n";
                    image.Source = new BitmapImage(new Uri("/Resources/UM.jpg", UriKind.Relative));  // логотип Угадай Мелодии
                    // Перемещаем ScrollViewer в начало
                ...
                case 9:
                    ...
                    PlayGifAnimation("/Resources/9.gif");
                    scrollViewer.ScrollToTop();
                    break;
                case 10:
                    // Интервалы раундов
                    ...
                    image.Source = new BitmapImage(new Uri("/Resources/10.png", UriKind.Relative));
                    scrollViewer.ScrollToTop();
                    break;
                case 11:
                    ...
                    image.Source = new BitmapImage(new Uri("/Resources/11.png", UriKind.Relative));
                    scrollViewer.ScrollToTop();
                    break;
                case 12:
                    // Горячие клавиши
                    ...
                    image.Source = new BitmapImage(new Uri("/Resources/12.png", UriKind.Relative));
                    scrollViewer.ScrollToTop();
                    break;
                case 13:
                    // Интернет
                    ...
                    image.Source = new BitmapImage(new Uri("/Resources/13.png", UriKind.Relative));
                    scrollViewer.ScrollToTop();
                    break;
                case 14:
                    // Серверная часть
                ...
                case 16:
                    ...
                    image.Source = new BitmapImage(new Uri("/Resources/16.png", UriKind.Relative));
                    scrollViewer.ScrollToTop();
                    break;
                case 17:
                    ...
                    image.Source = new BitmapImage(new Uri("/Resources/17.png", UriKind.Relative));
                    scrollViewer.ScrollToTop();
                    break;
                case 18:
                    ...
                    image.Source = new BitmapImage(new Uri("/Resources/18.png", UriKind.Relative));
                    scrollViewer.ScrollToTop();
                    break;
                case 19:
                    // Дополнительная информация
                    Info_last();
                    image.Source = new BitmapImage(new Uri("/Resources/19.png", UriKind.Relative));
                    scrollViewer.ScrollToTop();
                    break;
            }
        }
        ...
        private void PlayGifAnimation(string imagePath)
        {
            // Создание экземпляра BitmapImage
            var bitmap = new BitmapImage();
            bitmap.BeginInit();
            bitmap.UriSource = new Uri(imagePath, UriKind.Relative);
            bitmap.EndInit();

            // Добавление анимации GIF к изображению
        ...
        private void Info_last()
        {
            // Получаем текущую сборку приложения
            Assembly assembly = Assembly.GetEntryAssembly();
            // Получаем информацию о версии сборки
            FileVersionInfo fileVersionInfo = FileVersionInfo.GetVersionInfo(assembly.Location);
            text.Text = $"Информация о ПО:\n\nАвтор: PitBul477\nВерсия программы: {fileVersionInfo.FileVersion}\n\nПоддержать можно по следующей ссылке:\n";
            ...
            text.Inlines.Add(hyperlink);
            text.Inlines.Add($"\n\nОсобая благодарность sweet_rabbit_ за редактуру текста.");
        }
